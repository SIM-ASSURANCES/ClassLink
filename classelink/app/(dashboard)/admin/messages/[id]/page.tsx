...
export default async function AdminMessageDetailPage({ params }: Props) {
  ...
  const replyUrl = `/admin/messages/compose?recipient_id=${msg.sender_id}&subject=${encodeURIComponent(replySubject)}`

  return (
    <div>
      <div className="flex items-center gap-3 mb-6">
        <Link href="/admin/messages" className="text-sm text-gray-500 hover:text-gray-700 transition">
          ← Retour
        </Link>
      </div>
